...
def _wedges_pairs_number(hydr_head: float, frame_width: float, gate_height: float) -> int:
...
def _hydr_force(frame_width: float, gate_height: float, hydr_head: float, liquid_density: float,
                tilt_angle: float) -> float:
    head_top = hydr_head - gate_height * sin(tilt_angle)
    head_bottom = hydr_head
    return liquid_density * G * (head_top + head_bottom) / 2 * frame_width * gate_height


# old version:
# if kind == SlgKind.flow: result = 0
def _sealing_compress_force(kind: SlgKind, frame_width: float, gate_height: float) -> float:
...
def _screw_length(have_nodes_in_frame: bool, frame_height: float, way: float) -> float:
    ...
    if have_nodes_in_frame:
        return way + nut_height + 0.05
    return frame_height - way + nut_height


def _min_axial_force(gate_force: float, wedges_angle: float, screws_number: int) -> float:
...
def _way(gate_height: float, frame_height: float, optimal_frame_height: float) -> float:
...
def _min_close_moment(min_axial_force: float, screw: Screw) -> float:
    return min_axial_force * tan(SCREW_FRIC_ANGLE + screw.thread_angle) * screw.pitch_diam / 2
...
def _auma(required_torque: float, required_speed: float, job_mode: MotorMode, kind: SlgKind,
          screws_number: int) -> Auma:
...
def _check_screw(min_screw_inertia_moment: float) -> Screw:
...
def _nut_axe(axial_force: float) -> float:
    ...
    return (4 * axial_force / axes_number / pi / LIMIT_SHEAR_STRESS + hole_diam**2)**0.5
...
def _bolt_by_minor_diam(minor_diam: float, minimal_bolt: float,
    ...
    for i in METRICS:
        if i.major_diam < minimal_bolt or (only_recommend and not i.is_recommend):
            continue
...
def _wedge_by_bolt(bolt: MetricScrew) -> Wedge:
    try:
        result = next(filter(lambda i: i.bolt.major_diam >= bolt.major_diam, WEDGES))
...
class Slidegate():
    def __init__(self,
        ...
        if way is None:
            self.way = _way(self.gate_height, self.frame_height, self.optimal_frame_height)
        else:
            self.way = way

        if beth_frame_top_and_gate_top is None:
            self.beth_frame_top_and_gate_top = self.frame_height - self.gate_height
        else:
            self.beth_frame_top_and_gate_top = beth_frame_top_and_gate_top

        if self.frame_height < _minimal_frame_height_for_nodes(self.gate_height):
        ...
        self.min_screw_minor_diam = diam_by_inertia_moment(self.min_screw_inertia_moment)
        ...
        min_torque_in_one_screw = _min_close_moment(self.min_axial_force_in_one_screw, self.screw)
        ...
        if self.drive == Drive.electric:
            ...
            self.sleeve_sa = _sleeve_sa(self.screws_number, self.is_screw_pullout)

            self.torque_in_drive = max(self.auma.min_torque + DELTA_OPEN * self.screws_number,
                                       min_torque_in_drive)
        ...
        if screw_diam is None:
            self.min_screw_inertia_moment = min_inertia_moment(
                self.axial_force_in_one_screw, RECOMMEND_FOS, self.screw_length, SCREW_ELAST,
                SCREW_MJU)
            new_screw = _check_screw(self.min_screw_inertia_moment)
        ...
        if self.kind != SlgKind.flow:
            common_axial_force = self.screws_number * self.axial_force_in_one_screw
            self.min_bolts_by_all_pairs = _bolt_by_minor_diam(diam_circle_by_force_shear(
                common_axial_force / self.wedges_pairs_number / ONE_WEDGE_BOLTS_NUMBER,
                LIMIT_SHEAR_STRESS), 0.008, True)

            # only by TWO wedge-pairs
            self.wedge = _wedge_by_bolt(_bolt_by_minor_diam(diam_circle_by_force_shear(
                common_axial_force / 2 / ONE_WEDGE_BOLTS_NUMBER, LIMIT_SHEAR_STRESS), 0.008, True))
